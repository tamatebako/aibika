...
* Aibikawill load your script (using `Kernel#load`) and build
  the executable when it exits.

* Your program should 'require' all necessary files when invoked without
  arguments, so Aibikacan detect all dependencies.
...
* The 'current working directory' is not changed by Aibikawhen running
...
* The temporary location of the script can be obtained by inspected
  the `$0` variable.
* Aibikadoes not set up the include path. Use `$:.unshift File.dirname($0)`
...
* Avoid modifying load paths at run time. Specify load paths using -I
  or RUBYLIB if you must, but don't expect Aibikato preserve them for
  runtime. Aibikamay pack sources into other directories than you
  expect.
...
Aibikafirst runs the target script in order to detect any files that
are loaded and used at runtime (Using `Kernel#require` and
`Kernel#load`).

Aibikaembeds everything needed to run a Ruby script into a single
...
programs. The Aibikascript generates this executable and the
instructions to be run when it is launched.

When executed, the Aibikastub extracts the Ruby interpreter and your
...
in the executable unless the code is actually run when Aibikainvokes
your script. Otherwise, Aibikawon't know about it and will not include
the source files.

RubyGems are handled specially. Whenever a file from a Gem is
detected, Aibikawill attempt to include all the required files from
...
tree, since Aibikamay place the files elsewhere when extracted into the
temporary directory.

In case your script (or any of its dependencies) sets up autoloaded
module using `Kernel#autoload`, Aibikawill automatically try to
...
Aibikaexecutables clear the `RUBYLIB` environment variable before your
script is launched. This is done to ensure that your script does not
use load paths from the end user's Ruby installation.

Aibikaexecutables set the `RUBYOPT` environment variable to the value it
had when you invoked OCRA. For example, if you had `"RUBYOPT=rubygems"`
on your build PC, Aibikaensures that it is also set on PC's running the
executables.

Aibikaexecutables set OCRA_EXECUTABLE to the full path of the
...
The Aibikaexecutable does not change the working directory when it is
...
You can detect whether Aibikais currently building your script by
looking for the 'Ocra' constant. If it is defined, Aibikais currently
...
You can add additional files to the Aibikaexecutable (for example
...
Aibikawill include all files found below that directory. Glob patterns
...
By default, Aibikabuilds console application from `.rb` files and
...
By default, or if the `--console` option is used, Aibikawill
use the console runtime (`ruby.exe`). Aibikawill automatically select the
...
first check if your script works without Aibikausing `rubyw.exe`. A
